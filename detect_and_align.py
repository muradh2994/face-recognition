...
class Network(object):
    ...
    def fc(self, inp, num_out, name, relu=True):
        with tf.compat.v1.variable_scope(name):
            ...
            if input_shape.ndims == 4:
                ...
                feed_in, dim = (inp, input_shape[-1])  #int value has not attribute value.hence removing .value
    ...
    def softmax(self, target, axis, name=None):
        max_axis = tf.reduce_max(target, axis, keepdims=True) # ch: keep_dims is removed after tf1.7
        target_exp = tf.exp(target - max_axis)
        normalize = tf.reduce_sum(target_exp, axis, keepdims=True)
        softmax = tf.truediv(target_exp, normalize, name) # ch: div changed to truediv
        return softmax
